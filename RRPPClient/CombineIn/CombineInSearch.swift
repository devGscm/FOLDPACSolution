...
class CombineInSearch: BaseViewController, UITableViewDataSource, UITableViewDelegate
{
	
	@IBOutlet weak var tvCombineInSearch: UITableView!
	...
	func initViewControl()
	{
		...
		if(self.workType == Constants.RESALE_TYPE_RETURN)
		{
			lblSearchValue.text			= NSLocalizedString("resale_release_cust_name", comment: "반납처")
			lblResaleBranchName.text	= NSLocalizedString("resale_release_cust_name", comment: "반납처")
		}
		else
		{
	...
	func initDataClient()
	{
		...
		clsDataClient.addServiceParam(paramName: "resaleType", value:self.workType)
	}
	...
	func doSearch()
	{
		...
		tvCombineInSearch?.showIndicator()
		clsDataClient.addServiceParam(paramName: "startOrderReqDate", value: strLocaleStDate)
		clsDataClient.addServiceParam(paramName: "endOrderReqDate", value: strLocaleEnDate)
		...
		clsDataClient.selectData(dataCompletionHandler: {(data, error) in
			if let error = error {
				// 에러처리
				DispatchQueue.main.async { self.tvCombineInSearch?.hideIndicator() }
			...
			{
				self.tvCombineInSearch?.reloadData()
				self.tvCombineInSearch?.hideIndicator()
			}
		})
	...
	func scrollViewDidScroll(_ scrollView: UIScrollView)
	{
		...
		if boolBottom == true && tvCombineInSearch.isIndicatorShowing() == false
		{
			doSearch()
		}
	}
	...
	{
		let objCell:CombineInSearchCell = tableView.dequeueReusableCell(withIdentifier: "tvcCombineInSearch", for: indexPath) as! CombineInSearchCell
		...
		let strUtcOrderReqDate = clsDataRow.getString(name:"orderReqDate")
		if(strUtcOrderReqDate?.isEmpty == false)
		{
			let strLocaleOrderReqDate = DateUtil.utcToLocale(utcDate: strUtcOrderReqDate!, dateFormat: "yyyyMMddHHmmss")
			let strOrderReqDate = DateUtil.getConvertFormatDate(date: strLocaleOrderReqDate, srcFormat: "yyyyMMddHHmmss", dstFormat:"MM-dd")
			objCell.lblOrderReqDate.text = strOrderReqDate
		}
	...
	@objc func onSelectionClicked(_ sender: UIButton)
	{
		...
		let strtData = ReturnData(returnType: "combineInSearch", returnCode: nil, returnMesage: nil, returnRawData: clsDataRow)
