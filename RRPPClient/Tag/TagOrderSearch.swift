...
class TagOrderSearch: BaseViewController, UITableViewDataSource, UITableViewDelegate
{
	
	@IBOutlet weak var tvTagOrderSearch: UITableView!
	...
	func doSearch()
	{
		...
		tvTagOrderSearch?.showIndicator()
		...
		clsDataClient.selectData(dataCompletionHandler: {(data, error) in
			if let error = error {
				// 에러처리
				DispatchQueue.main.async { self.tvTagOrderSearch?.hideIndicator() }
	...
	func scrollViewDidScroll(_ scrollView: UIScrollView)
	{
		...
		if boolBottom == true && tvTagOrderSearch.isIndicatorShowing() == false
		{
			doSearch()
		}
	}
	...
	{
		let objCell:TagOrderSearchCell = tableView.dequeueReusableCell(withIdentifier: "tvcTagOrderSearchCell", for: indexPath) as! TagOrderSearchCell
		...
		objCell.lblIssueOrderId.text = clsDataRow.getString(name:"issueOrderId")
		objCell.lblAssetEpcName.text = clsDataRow.getString(name:"assetEpcName")
		objCell.lblDeliBranchName.text = clsDataRow.getString(name:"deliBranchName")
	...
	@objc func onSelectionClicked(_ sender: UIButton)
	{
		...
		let strtData = ReturnData(returnType: "tagOrderSearch", returnCode: nil, returnMesage: nil, returnRawData: clsDataRow)
